...
public class ListEx {
	...
	public static void main(String[] args) {
		...
		//1을 제거
		list.remove((Integer)1); //첫번째 만나는 1을 제거하고 제거가 되어서 true를 반환 /Integer를 안붙이면, int로 인식이됨 
		System.out.println(list);
		//1번지에 있는 객체를 제거(3을 제거)
		Integer num = list.remove(1); //1번지에 있는 요소를 제거하고 제거한 요소를 반환
		System.out.println(num);
		System.out.println(list);
		System.out.println(list.size()); //저장된 객체 수를 출력. 저장 가능한 최대 객체 수가 아님/변수를 따로 저장할 필요가 없음
		...
		while(it.hasNext()) { //hasNext: 다음요소가 있는지 없는지 / 있으면 다음 요소를 가지고와서 저장, 없으면 종료
			Integer tmp = it.next();
			System.out.print(tmp + " ");
		}
		System.out.println();
