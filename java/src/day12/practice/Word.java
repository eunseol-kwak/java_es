...
public class Word {
	...
	private String meaning[]; //뜻을 여러개 저장하기 위해 배열로 만듬
	...
	public Word(String title) {
		this.title = title;
		this.meaning = new String[5]; //뜻을 넣지 않아도 배열을 만들어 주는게 좋음?
	}
	//복사생성자
	public Word(Word w) {
		this.title = w.title;
	...
	public void print() {
		...
		for(int i = 0; i < meaningCount; i++) {  //저장된 개수만 확인=> 조건문 사라짐 if(meaning[i] != null)
			System.out.println(i+1+"." +meaning[i]);
		}
	}
	/**뜻을 추가하는 메서드
	 * 매개변수: 추가될 뜻 => String meaning
	 * 리턴타입: 없음 = void /멤버의 값을 바꾸는것이어서 없음
	 * 메서드명: addMeaning
	 * */
