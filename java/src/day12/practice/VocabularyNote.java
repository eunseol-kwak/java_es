...
public class VocabularyNote {
	...
	public VocabularyNote(Word wordList[]) {
		...
		for(int i = 0; i < wordList.length; i++) {
			...
			//this.wordList[i] = wordList[i]; //얕은복사
		}
	...
	public void insert(Word word) {
		...
		if(wordCount == wordList.length) {
			System.out.println("Vocabulary nite is full!");
			return;
		}
		//깊은 복사를 위해 Word의 복사 생성자를 이용하여 새 단어를 생성한 후 추가
		//객체는 얕은 복사를 하면 문제가 생길 수 있음
		wordList[wordCount++] = new Word(word);
	}
	
	/**단어와 뜻이 주어지면 단어장에 추가하는 메서드(단어와 뜻을 넘겨주는 경우)/메서드 오버로딩
	 ...
	 * 리턴타입: (boolean으로해서 사용자에게 알려줄 수 있음)없음 => void
	 * 메서드명: delete
	 * */
	public void delete(String title) {
		//단어가 어디 있는지 찾아야 함
		int index = indexOf(title);
		//단어장에 단어가 없으면 알림 메세지 출력 후 종료
		if(index == -1) {
		...
		//단어의 개수를 하나 줄여줌
		wordCount--;
		//마지막 단어를 비워줌(null)/앞으로 한칸 당기면 뒤에있는애가 두번쓰여서 순서를 바꿔야함
		wordList[wordCount] = null;
	}
	
	/**단어가 주어지면 단어있는 위치를 알려주는 메서드
	 * 딘어가 없으면 -1 리턴
	...
	/**단어와 수정할 뜻의 번호와 수정할 뜻이 주어지면 단어의 '뜻을 수정'하는 메서드
	 * 매개변수: 단어, 수정 할 뜻 번호, 수정할 뜻 => String title, int meaningIndex, String meaning
	 * 리턴타입: 없음 => void (수정하고 끝내면 됨)
	 * 메서드명: updateMeaning
	 * */
	...
	/**단어와 수정할 단어가 주어지면 '단어를 수정'하는 메서드
