...
public class BaseballGameEx {
	...
	public static int strike(int arr1[], int arr2[]) {
		//배열 생성이 안된 배열이 1라도 있으면 비교할 수 없어서 0을 리턴/배열이 비어있는지 고려
		if(arr1 == null || arr2 == null) {
			return 0;
		}
		//배열의 개수가 다른 경우 작은 크기의 배열을 기준으로 비교하기 위해 size계산/두개의 개수가 다를수 있는지 고려
		// 1 2 3
		// 4 5 6 7 /스트라이크는 오류, 볼은 상관없음
		int size = arr1.length < arr2.length ? arr1.length : arr2.length;
		int count = 0; //스트라이크 개수
		for(int i = 0; i < size; i++) {
			//같은 번지에 있는 값들이 서로 같으면(스트라이크이면 개수를 1증가)
			if(arr1[i] == arr2[i]) {
				count++;
			}
		}
	...
	public static int ball(int arr1[], int arr2[]) {
		...
		int count = 0;
		//다른 번지를 비교해서 찾아도 되지만
		//두 배열에서 같은 숫자의 개수에서 스트라이크 개수를 빼면 볼의개수
		for(int tmp : arr1) {
			//arr1에서 꺼낸 값이 arr2에 있으면 개수를 증가
