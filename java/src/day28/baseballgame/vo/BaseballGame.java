...
public class BaseballGame {
	...
	public boolean generate(int size, int min, int max) {
		...
		//max가 min보다 작으면
		if(max < min) { 
			return false; //return처리 안하고 max랑 min이랑 바꿔도 괜찮음
		}
		...
		while(set.size() < 3) {
			set.add(random.nextInt(max - min + 1) + min); //Math.random(); 사용해도 괜찮음
		}
		//중복되지 않게 처리된 set을 List에 저장
		com.clear();	 //그전에 생성했던 숫자는 지워줘야함, 기존것 삭제
		com.addAll(set); //addAll: 여러개 한꺼번에 처리
		Collections.shuffle(com);  //shuffle: 순서를 알아서 섞어줌 /리스트를 섞어줌
		return true;
	}
	...
	public boolean setUser(List<Integer> user) {
		...
		for(Integer tmp1 : this.user) {
			//범위를 벗어난 값이 있으면
			if(tmp1 < 1 || tmp1 > 9) { //복사해서 같기때문에 한쪽만 체크해도 괜찮음(tmp2X)
				return false;
			}
			int count = 0;
