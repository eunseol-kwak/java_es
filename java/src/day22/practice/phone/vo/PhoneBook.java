...
public class PhoneBook {
	...
	public boolean updateName(String name, String updateName) {
		...
		//가져온 전화번호의 이름을 수정
		pn.setName(updateName); //참조변수는 setter를통해 멤버들의 값이 변경가능
		//list.get(index).setName(updateName); //위의 두줄을 한줄로
		return true;
	...
	public boolean updateNumber(String name, String number) {
		...
		//전화번호 패턴이 맞으면 전화번호 객체를 생성 했을때 전화번호가 있고, 
		//패턴이 맞지 않으면 빈 문자열이 있다) 
		//name과 number를 이용하여 전화번호 객체를 생성하는 대신 checkNumber(public static으로 변경)를 이용
