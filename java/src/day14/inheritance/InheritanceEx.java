...
public class InheritanceEx {
	...
	public static void main(String[] args) {
		...
		//문법적으로 에러가 발생하지 않지만, 예외가 발생 : 다운캐스팅이 안되는 경우이기 때문에
...
class Phone {
	...
	//public Phone(){} //생성자가 하나도 없으면 컴파일할때 기본 생성자가 자동으로 추가
	
}
class SmartPhone extends Phone{
	//스마트폰은 카메라를 가지고있다(has a 관계, 멤버로)
	...
	public SmartPhone() {
		//부모 클래스의 생성자가 호출
		//super()를 이용하여 부모 클래스의 생성자를 직접 호출하지 ㅇ낳으면
		//기본 생성자인 super()가 자동으로 추가가 되고 호출이됨.
		//이 때, 부모클래스의 기본 생성자가 없으면 에러가 발생
		//super(); //에러 발생
		super("");
	}
	@Override
	public void call(String num) {//String num2쓰면 오버라이딩이 아니라 오버로딩이라 가능
		super.call(num); //부모클래스와 동일/ 잘 안쓰임
		/*System.out.println(num); 
		System.out.println("Calling..."); //오버라이딩 */
		System.out.println("SmartPhone!");
	}	
}		
class Camera{
	public void take() {
		System.out.println("take a puicture!");
	}
}